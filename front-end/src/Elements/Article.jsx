  ...
  return (
    <>
      <Link to={`/article/${data.id}`}>
        <h1 className='article_short_title'>{data.title.substring(0, 25)}...</h1>
      </Link>
      {data.description.split('\n').map((str, i) => (
        <Description key={i} limit={98} string={str} />
      ))}
      {/* <h1 className='article_short_description'>{data.description.substring(0, 98)}...</h1>
